...
class GNN(BaseGNN):
  def __init__(self, opt, dataset, device=torch.device('cpu')):
    ...
    self.output_norm = nn.LayerNorm(opt['hidden_dim'])
    if opt['dataset'] in ['questions','workers','minesweeper']:
      self.output_linear = nn.Linear(opt['hidden_dim'], 1)
  ...
  def forward(self, x, pos_encoding=None):
    ...
    if self.opt['dataset'] in ['minesweeper', 'workers', 'questions']:
