...
def get_dataset(opt: dict, data_dir, use_lcc: bool = False, split=0) -> InMemoryDataset:
  ...
  #   features = dataset.features  # Access node features
    ...
    dataset = GitHub(root=path, transform=None)
    use_lcc = False  # never need to calculate the lcc with ogb datasets
    # get train and test mask

    train_size = int(dataset.data.x.shape[0] *0.7)
    test_size = int(dataset.data.x.shape[0] *0.2)
    val_size = int(dataset.data.x.shape[0] *0.1)
    dataset.data = get_train_val_test_split(random_state=12345, data=dataset.data,
                                            train_examples_per_class=None,
                                            val_examples_per_class=None,
                                            test_examples_per_class=None, train_size=train_size,
